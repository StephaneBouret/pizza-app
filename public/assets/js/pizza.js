...
(function () {
  ...
  document.addEventListener("click", async (e) => {
    ...
    if (state.sizes.length) {
      state.sizes.forEach((s) => {
        ...
        pmSizes.insertAdjacentHTML(
          "beforeend",
          `
          <div class="form-check me-3 mb-2">
            <input class="form-check-input" type="radio" name="size" id="${rid}" value="${
            s.label
          }" ${s.label === "L" ? "checked" : ""}>
      ...
      pmSizes.innerHTML =
        '<div class="text-muted small">Aucune taille disponible.</div>';
    }

    // --- Ingrédients additionnels (Select multiple trié) ---
    pmExtras.innerHTML = "";
    if (state.extras.length) {
      state.extras.sort((a, b) =>
        a.name.localeCompare(b.name, "fr", { sensitivity: "base" })
      );
      state.extras.forEach((ex) => {
        const price = parseInt(ex.extraPriceCents, 10) || 0;
        const label = `${ex.name} (+${centsToEuro(price)} €)`;
        pmExtras.insertAdjacentHTML(
          "beforeend",
          `<option value="${ex.id}" data-price="${price}">${label}</option>`
        );
      });
    } else {
      pmExtras.insertAdjacentHTML(
        "beforeend",
        `<option disabled>Aucun ingrédient additionnel disponible.</option>`
      );
    }
  ...
  form?.addEventListener("submit", async (e) => {
    ...
    if (out && out.ok) {
      // Mise à jour du badge en direct si le serveur renvoie "count"
      if (typeof out.count === "number") {
        updateCartBadge(out.count);
      }
      // Redirection (ou non)
  ...
  // --- Rafraîchit la pastille du panier dans la navbar ---
  ...
  // --- Appelle le serveur pour connaître le nombre d'articles distincts ---
  async function refreshCartBadgeFromServer() {
    try {
      ...
      console.error("Erreur lors de la mise à jour du badge panier :", err);
    }
  }

  // --- À l'ouverture de page, met à jour la pastille (utile si ajout depuis une autre vue) ---
  document.addEventListener("DOMContentLoaded", refreshCartBadgeFromServer);
})();
